...
// Patched for rust-lang/rust#74840: an extern static of an uninhabited type
// (`enum Class {}`) is being phased out. An opaque zero-sized struct keeps the
// same "never constructed, only pointed at" semantics and is inhabited.
#[repr(C)]
struct Class {
    _priv: [u8; 0],
}
